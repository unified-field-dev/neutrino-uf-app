//! Mount Neutrino secrets pages for Playwright.
...
use crate::neutrino_routes_eager::NeutrinoRoutesEager;
...
/// Root: harness auth + eager secrets routes.
#[component]
pub fn App() -> impl IntoView {
    ...
    #[cfg(feature = "ssr")]
    {
        ...
        crate::wire_gauge_permissions_bridge();
    }
    provide_shell_auth_menu(|| view! { <HarnessAuthMenu /> });

    view! {
        <OrbitalTemplate>
            <Stylesheet id="leptos" href="/pkg/neutrino-uf-app-e2e.css"/>
            <Title text="neutrino-uf-app e2e"/>
            <E2eAuthProvider>
                <Router>
...
fn HomePage() -> impl IntoView {
    view! {
        <UnifiedFieldShellLayout>
            <ShellAppBar slot>
                <UnifiedFieldAppBar app_name="Neutrino e2e".to_string()>
                    <ShellAuthMenu slot:auth_menu>
                        <HostAuthMenu />
                    </ShellAuthMenu>
                </UnifiedFieldAppBar>
            </ShellAppBar>
            <ContentContainer max_width="900px" data_testid="neutrino-e2e-home">
                <Flex vertical=true gap=FlexGap::Medium align=FlexAlign::Start>
                    <Title3>"neutrino-uf-app e2e"</Title3>
                    <Body1>"NeutrinoRoutes host for Playwright."</Body1>
                    <Link href="/secrets">"Open /secrets"</Link>
                </Flex>
            </ContentContainer>
        </UnifiedFieldShellLayout>
    }
